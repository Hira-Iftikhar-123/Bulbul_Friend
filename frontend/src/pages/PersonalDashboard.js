  ...
  const processAudio = async (audioBlob, isFinal = false) => {
    try {
      ...
      const endpoint = selectedProvider === 'openai'
        ? `${process.env.REACT_APP_API_URL || 'https://bulbulfriend-backend.up.railway.app'}/api/openai`
        : `${process.env.REACT_APP_API_URL || 'https://bulbulfriend-backend.up.railway.app'}/api/gemini-process`; 
